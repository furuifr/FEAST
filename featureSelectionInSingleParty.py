...
def featureSelection(new_col_num, data, data_dsct, target, method, dataset_name, dsct_method, all_columns, RANDOMSEED, subdir):
    model = dataset_name+'_'+dsct_method+'_'+str(new_col_num)
    ...
    for i in range(new_col_num):
        ...
        if i==0:
            ...
            acc_res_fl_ratio_logi.append(clf_cv('logi', data_selected.copy().T.drop_duplicates().T, target, RANDOMSEED))
            acc_res_fl_ratio_svc.append(clf_cv('svc', data_selected.copy().T.drop_duplicates().T, target, RANDOMSEED))
            acc_res_fl_ratio_rfc.append(clf_cv('rfc', data_selected.copy().T.drop_duplicates().T, target, RANDOMSEED))
            acc_res_fl_ratio_xgbc.append(clf_cv('xgbc', data_selected.copy().T.drop_duplicates().T, target, RANDOMSEED))
